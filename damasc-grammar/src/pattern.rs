...
pub fn single_pattern<'s>() -> (Boxed<'s, 's, &'s str, Pattern<'s>, extra::Err<Rich<'s, char>>>, Recursive<Indirect<'s, 's, &'s str, Expression<'s>, extra::Err<Rich<'s, char>>>>) {
    ...
    let pattern_declartion = recursive(|pattern| {
        ...
        let capture = single_identifier().then(just("@").ignore_then(pattern.clone()).or_not()).boxed().map_with(|(id, pat), meta| {
            if let Some(p) = pat {
        ...
        let array = pattern.clone().map(ArrayPatternItem::Pattern)
            ...
            .allow_trailing()
            .collect()
            .then(just("...").padded().ignore_then(pattern.clone().map(Box::new).map(Rest::Collect).or_not().map(|s| s.unwrap_or(Rest::Discard))).or_not())
            .padded()
            .delimited_by(
                just('['),
                just(']')
            ...
            .map_with(|(elements, rest), meta| {
                Pattern::new_with_location(PatternBody::Array(elements, rest.unwrap_or(Rest::Exact)), meta_to_location(meta))
            })
            .labelled("array")
            .as_context()
            .boxed();
        ...
        let member = 
            choice((
                ...
                expression_declaration.clone().padded().delimited_by(just("["), just("]")).map(PropertyKey::Expression),
            ))
            .labelled("object_key")
            .as_context()
        ...
        let object = member
            ...
            .map_with(|(entries, rest), meta| {
                Pattern::new_with_location(PatternBody::Object(entries, rest.unwrap_or(Rest::Exact)), meta_to_location(meta))
            })
            .labelled("object")
            .as_context()
            .boxed();
