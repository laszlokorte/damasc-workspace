...
use nom::{
    branch::alt,
    bytes::complete::tag,
    combinator::{all_consuming, map, value, cut},
...
pub(crate) fn command<'a, 'b, 's, E:ParserError<'s>>(input: ParserInput<'s>) -> ParserResult<Command<'a, 'b>, E> {
    context("command", alt((
        ...
        value(Command::Help, all_consuming(context("cmd_help", alt((tag(".help"), tag(".h")))))),
        value(Command::Exit, all_consuming(context("cmd_exit", alt((tag(".exit"), tag(".quit"), tag(".q")))))),
        value(Command::ShowEnv, all_consuming(context("cmd_env", alt((tag(".env"), tag(".e")))))),
        value(Command::ClearEnv, all_consuming(context("cmd_clearenv", alt((tag(".clearenv"), tag(".ce")))))),
        cut(alt((
            context("cmd_transform", map(all_consuming(transformation), Command::Transform)),
            context("cmd_let", map(all_consuming(preceded(ws(tag("let ")), assignment_set1)), Command::Assign)),
            context("binding", map(all_consuming(assignment_set1), Command::Match)),
            context("binding_expressions", map(all_consuming(
                pair(
                    expression_many1,
            ...
            context("expressions", all_consuming(map(
                expression_many1,
