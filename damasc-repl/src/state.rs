use damasc_lang::{value::ValueBag, runtime::{env::Environment, evaluation::EvalError, matching::{Matcher, AssignmentError}}};
    ...
    pub fn eval(&mut self, command: Command<'s,'s>) -> Result<ReplOutput<'i, 's, 'v>, ReplError> {
        match command {
            ...
            Command::Transform(transformation) => {
                ...
                let trans_iterator = MultiProjectionIterator::new(self.environment.clone(), transformation.projection, iter);
                
                let transform_result = trans_iterator.flatten().flatten().collect::<Vec<_>>();

                Ok(ReplOutput::Values(ValueBag{values: transform_result}))
            },
            Command::Assign(assignments) => {
                let matcher = Matcher::new(&self.environment);
                match matcher.eval_assigment_set(assignments) {
                    Ok(new_bindings) => {
                        self.environment.bindings.append(&mut new_bindings.bindings.clone());
                        Ok(ReplOutput::Bindings(new_bindings))
                    },
                    Err(AssignmentError::EvalError) => {
                        Err(ReplError::EvalError)
                    },
                    Err(AssignmentError::MatchError) => {
                        Err(ReplError::MatchError)
                    },
                    Err(AssignmentError::TopologyError) => {
                        Err(ReplError::TopologyError)
                    },
                }
            },
            ...
            Command::Eval(assignments, expresions) => {
                ...
                let mut new_bindings = match matcher.eval_assigment_set(assignments) {
                    ...
                    Err(AssignmentError::MatchError) => {
                        return Err(ReplError::MatchError)
                    },
                    Err(AssignmentError::TopologyError) => {
                ...
                let values = expresions.expressions.into_iter()
